// Force redeploy - 2025-01-07 - Removed transactions (not supported)
      ...
      try {
        ...
        if (sheet.orders && Array.isArray(sheet.orders) && sheet.orders.length > 0) {
          const pedidosValues = sheet.orders.map((order, orderIndex) => ({
            id: order.id || `${planillaId}-order-${orderIndex + 1}`,
          ...
          await sql`INSERT INTO pedidos ${sql(pedidosValues, 'id', 'planilla_id', 'secuencia', 'cliente', 'direccion', 'telefono', 'barrio', 'total', 'estado', 'observaciones')}`;
          console.log(`[API] ✓ ${sheet.orders.length} pedidos insertados`);
          ...
          if (allProductos.length > 0) {
            await sql`INSERT INTO pedido_productos ${sql(allProductos, 'pedido_id', 'codigo', 'nombre', 'categoria', 'cantidad', 'precio_unitario', 'total', 'devuelto')}`;
            console.log(`[API] ✓ ${allProductos.length} productos insertados`);
          }
        }
        
        insertCount++;
        createdPlanillas.push({ id: planillaId, ruta: sheet.ruta, fecha: sheet.fecha, pedidos: sheet.orders?.length || 0 });
...
export async function GET(request: NextRequest) {
  try {
    ...
    const planillas = await sql`
      SELECT p.id, p.fecha, p.tipo_ruta, p.entregador, p.total_cargue, p.total_entregado,
        p.total_fiado, p.total_repaso, p.total_devolucion, p.estado, p.observaciones,
        p.created_at, p.updated_at,
        COALESCE(json_agg(json_build_object(
          'id', ped.id, 'planilla_id', ped.planilla_id, 'secuencia', ped.secuencia,
          'cliente', ped.cliente, 'direccion', ped.direccion, 'telefono', ped.telefono,
          'barrio', ped.barrio, 'total', ped.total, 'estado', ped.estado,
          'observaciones', ped.observaciones,
          'productos', (SELECT COALESCE(json_agg(json_build_object(
            'codigo', pp.codigo, 'nombre', pp.nombre, 'categoria', pp.categoria,
            'cantidad', pp.cantidad, 'precio_unitario', pp.precio_unitario,
            'total', pp.total, 'devuelto', COALESCE(pp.devuelto, false),
            'monto_entregado', pp.monto_entregado, 'monto_devuelto', pp.monto_devuelto
          ) ORDER BY pp.id), '[]'::json) FROM pedido_productos pp WHERE pp.pedido_id = ped.id)
        ) ORDER BY ped.secuencia) FILTER (WHERE ped.id IS NOT NULL), '[]'::json) as pedidos
      FROM planillas p
